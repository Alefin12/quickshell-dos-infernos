...
Rectangle {
    ...
    readonly property bool isFocusedMonitor: Hyprland.focusedMonitor && Hyprland.focusedMonitor.name === "DP-1"

    readonly property int targetContentWidth: {
        let w = 0;
        for (let i = 0; i < Hyprland.workspaces.values.length; i++) {
            const ws = Hyprland.workspaces.values[i];
            if (ws.monitor.name !== "DP-1" || ws.name === "special:magic")
                continue;
 // match your state widths
    ...
    border.color: !work2.isFocusedMonitor ? Colors.md3.on_primary_fixed : Colors.md3.primary_fixed_dim
    border.width: 1
    ...
    RowLayout {
        ...
        Repeater {
            ...
            Shape {
                ...
                visible: modelData.monitor.name === "DP-1" && modelData.name !== "special:magic"
